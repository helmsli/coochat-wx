...
public class BaseRedisService {
	 ...
	 * @param joinClubLockSeconds -- 锁定秒数
	 * @param joinClubWaitSeconds -- 等待锁的秒数，等待超过该时间，返回超时
	...
	public void releaseCommonLock(String lockKey, long requestTime) {
		try {
			String requestTimeS = (String) redisTemplate.opsForValue().get(lockKey);
			long requestTimeL = StringUtils.isEmpty(requestTimeS) ? 0 : Long.parseLong(requestTimeS);
